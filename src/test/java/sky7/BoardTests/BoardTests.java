...
public class BoardTests {
    ...
    public void isMovePossibalTest(){
        ...
        for (int i = 0; i < 100000; i++) {
            ...
            switch (oneInFour){
                case 0 : break; // Don't trun;
                case 1 : board.rotateRobot(0,1);
                        roboDir = turnRight(roboDir);break;
                case 2 : board.rotateRobot(0,2);
                        roboDir = roboDir.inverse(roboDir);break;
                case 3 : board.rotateRobot(0,-1);
                        roboDir = turnLeft(roboDir); break;
            ...
            int newY = currY;
            switch (roboDir){
                case NORTH: canGo = checkCanGo(board,roboDir, currX, currY+1); newY++;; break;
                case WEST: canGo = checkCanGo(board,roboDir, currX-1, currY); newX--;break;
                case SOUTH: canGo = checkCanGo(board,roboDir, currX, currY-1); newY--;break;
                case EAST: canGo = checkCanGo(board,roboDir, currX+1, currY); newX++;break;
            }
    ...
    private boolean checkCanGo(Board board, DIRECTION dir, int x, int y){
        ...
        for (ICell cell : tiles){
            if (cell instanceof Wall) {
                Wall wall = (Wall) cell;
                if(dir.inverse(dir) == wall.getDirection()){
                    canGo = false;
                }
            }
        }
    ...
    private DIRECTION turnRight(DIRECTION dir){
        switch (dir){
            ...
            default: throw new IllegalArgumentException("Dir: " + dir + "is not one of the four DIRECTIONs");
        }
    }
